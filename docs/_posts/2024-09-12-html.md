...
The following provides some steps for how I overcame this problem. Call it sharing my pain! 

All code referenced henceforth can be found in this [repo](https://github.com/gwilkinson01/ansible-devops).

### Split

There people out there with much better knowledge of Ansible who probably have a way better solution for this than my one, but anyway here is my inelegant method. 
...
Note that I needed to configure the password as an environment variable so that it can later be succesfully passed to the `hosts.ini` file that Ansible uses for authentication.

### Become

In my playbook I required a privilege escalation step. In Ansible, you can gain root privileges by using the become directive. This allows a non-root user to run commands with elevated privileges (such as root) by using sudo or similar methods.
...
If you're servers are using sudoexec you will have to create a role. That's a whole other thing that I won't go into here. Nevertheless, I've included a role in the repo just so you have an idea about how the tree structure might look if you did need that.
