...
Microsoft Defender External Attack Surface Management (MDEASM) is an Azure based security service that helps organizations gain visibility and manage their external attack surface. I was recently presented with the challenge of having to deploy and configure this resource entirely through code. What follows is a guide of how to do just that.
...
I used Terraform to define and provision MDEASM (and a reosource group for it to sit in). I won't detail anything about using Terraform with Azure DevOps here as there are many tutorials on that subject already. But here are the three Terraform files I used for deployment with some important notes:
...
Then create a YAML file to define the steps for the pipeline. Here is a basic example of what this might look like:
